...
class AdaIR(nn.Module):
    def __init__(self, 
        ...
        trust_hidden = max(dim // 2, 16)
        # Learn how much each spatial location should trust the temporal prior
        # versus the original degraded frame.
        self.trust_predictor = nn.Sequential(
            nn.Conv2d(inp_channels * 2 + 3, trust_hidden, kernel_size=3, stride=1, padding=1, bias=True),
            nn.GELU(),
            nn.Conv2d(trust_hidden, trust_hidden, kernel_size=3, stride=1, padding=1, bias=True),
            nn.GELU(),
            nn.Conv2d(trust_hidden, 1, kernel_size=1, stride=1, padding=0, bias=True),
        )

    def _compute_color_edge_strength(self, inp_img):
        gray = inp_img.mean(dim=1, keepdim=True)
        gx = torch.zeros_like(gray)
        gy = torch.zeros_like(gray)
        gx[:, :, :, 1:] = gray[:, :, :, 1:] - gray[:, :, :, :-1]
        gy[:, :, 1:, :] = gray[:, :, 1:, :] - gray[:, :, :-1, :]
        edge = torch.sqrt(gx * gx + gy * gy + 1e-12)
        edge_scale = edge.detach().flatten(1).amax(dim=1).view(-1, 1, 1, 1) + 1e-6
        return torch.clamp(edge / edge_scale, 0.0, 1.0)

    def _compute_temporal_guidance(self, inp_img, noise_emb, prior_strength=1.0):
        if noise_emb is None:
            return inp_img, None

        if noise_emb.dim() == 3:
            noise_emb = noise_emb.unsqueeze(0)

        if noise_emb.shape[-2:] != inp_img.shape[-2:]:
        ...
        if noise_emb.shape[1] < 3:
            repeat_factor = (3 + noise_emb.shape[1] - 1) // noise_emb.shape[1]
            noise_emb = noise_emb.repeat(1, repeat_factor, 1, 1)

        noise_emb = noise_emb[:, :3, :, :].to(dtype=inp_img.dtype)
        noise_emb = torch.clamp(noise_emb, 0.0, 1.0)

        prior_rgb = noise_emb[:, :inp_img.shape[1], :, :]
        residual = torch.abs(inp_img - prior_rgb).mean(dim=1, keepdim=True)
        residual_scale = residual.detach().flatten(1).amax(dim=1).view(-1, 1, 1, 1) + 1e-6
        residual = torch.clamp(residual / residual_scale, 0.0, 1.0)

        edge = self._compute_color_edge_strength(inp_img)
        edge_keep = torch.clamp(1.0 - 0.55 * edge, 0.25, 1.0)
        base_gate = residual * edge_keep

        trust_input = torch.cat([inp_img, prior_rgb, residual, edge, base_gate], dim=1)
        trust_map = torch.sigmoid(self.trust_predictor(trust_input))
        trust_map = trust_map * base_gate
        trust_map = trust_map * max(float(prior_strength), 0.0)
        trust_map = torch.clamp(trust_map, 0.0, 0.55)

        guided = inp_img * (1.0 - trust_map) + prior_rgb * trust_map
        return guided, trust_map

    def _build_temporal_guidance(self, inp_img, noise_emb, prior_strength=1.0):
        guided, _ = self._compute_temporal_guidance(inp_img, noise_emb, prior_strength=prior_strength)
        return guided

    def forward(self, inp_img,noise_emb = None, prior_strength = 1.0, return_aux = False):

        freq_guide, trust_map = self._compute_temporal_guidance(inp_img, noise_emb, prior_strength=prior_strength)
        if trust_map is None:
            model_input = inp_img
        else:
            base_mix = 0.04 * max(float(prior_strength), 0.0)
            encoder_mix = torch.clamp(base_mix + 0.18 * trust_map, 0.0, 0.22)
            model_input = inp_img * (1.0 - encoder_mix) + freq_guide * encoder_mix

        inp_enc_level1 = self.patch_embed(model_input)
        ...
        if self.decoder:
            latent = self.fre1(freq_guide, latent)
        ...
        if self.decoder:
            out_dec_level3 = self.fre2(freq_guide, out_dec_level3)
        ...
        if self.decoder:
            out_dec_level2 = self.fre3(freq_guide, out_dec_level2)
