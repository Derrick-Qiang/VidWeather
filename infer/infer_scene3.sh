...
SCENE_PATH="${SCENE_PATH:-data/test/deblur/3}"
OUTPUT_ROOT="${OUTPUT_ROOT:-output/3}"
...
PRIOR_CKPT="${PRIOR_CKPT:-AdaIR/snow_only_noprior_stage1_e30/epoch=29-step=24210.ckpt}"
STAGE1_OUTPUT=""
...
run_noprior_stage() {
  ...
  echo "Running stage 1: scene 3 without video prior..."
...
run_prior_stage() {
  ...
  local stage1_input="${STAGE1_OUTPUT}/deblur/3"
  ...
  echo "Final result saved to: ${FINAL_OUTPUT}/deblur/3"
}
