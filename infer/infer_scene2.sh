...
SCENE_PATH="${SCENE_PATH:-data/test/deblur/2}"
OUTPUT_ROOT="${OUTPUT_ROOT:-output/2}"
...
PRIOR_CKPT="${PRIOR_CKPT:-AdaIR/vp_adair_partial_trustsup_v1_e10/epoch=49-step=73000.ckpt}"
STAGE1_OUTPUT=""
...
run_noprior_stage() {
  ...
  echo "Running stage 1: scene 2 without video prior..."
...
run_prior_stage() {
  ...
  local stage1_input="${STAGE1_OUTPUT}/deblur/2"
  ...
  echo "Final result saved to: ${FINAL_OUTPUT}/deblur/2"
}
